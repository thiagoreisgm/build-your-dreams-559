...
function ConteudoPage() {
  ...
  return (
    <GSPage subtitle={
      <>
        Perfil de Conteúdo — calibra a IA e personaliza a busca de inspiração. Diferente do{" "}
        <span className="text-[var(--color-ink)]">ICP</span> (a quem você vende), aqui é{" "}
        <span className="text-[var(--color-ink)]">o que você posta</span> e usa como referência.
      </>
    }>
      ...
      <div className="mt-5">
        <Card title="Temas que você posta" hint="Alimenta a busca de inspiração">
          <div className="flex flex-wrap gap-2">
            {ALL_THEMES.map((t) => (
              <Chip key={t} label={t} active={profile.themes.includes(t)} onClick={() => toggle("themes", t)} />
            ))}
            <button
              type="button"
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <Card title="Formatos de referência" hint="Que tipo de post a IA deve buscar e imitar a estrutura" accent>
          <div className="flex flex-wrap gap-2">
            {ALL_FORMATS.map((f) => (
              <Chip key={f} label={f} active={profile.formats.includes(f)} onClick={() => toggle("formats", f)} />
            ))}
          </div>
        </Card>
      ...
      <div className="mt-5">
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-xs text-[var(--color-orange)]">{freedomLabel(profile.creative_freedom)}</span>
          </div>
          <input
            type="range"
            min={0}
            ...
            onChange={(e) => setProfile({ ...profile, creative_freedom: Number(e.target.value) })}
            className="w-full"
          />
